...
> **Infraestrutura: VPS única, auto-hospedada, com PostgreSQL na mesma máquina.**
...
## 5.10 Ambiente de desenvolvimento assistido — MCPs, plugins e conexões

Para que o próprio desenvolvimento seja automatizado. O ecossistema MCP muda rápido: **verifique nome, origem e permissões de cada servidor antes de instalar**, e nunca aponte um servidor MCP de terceiro para o banco de produção.

### 5.10.1 MCPs de desenvolvimento (na sua máquina, não em produção)

| Servidor | Para quê | Cuidado |
|---|---|---|
**PostgreSQL** | Inspeção de schema, `EXPLAIN`, validação de migration pelo agente | Aponte para **banco local ou cópia**, jamais produção |
**Filesystem** | Leitura e escrita no repositório | Escopo restrito ao diretório do projeto |
**Git / GitHub** | Branches, PRs, issues, revisão | Token com escopo mínimo |
**Playwright** | E2E dirigido pelo agente, inspeção visual das telas construídas | — |
**Sentry** | Puxar erro de produção e propor correção | Token somente-leitura |
**Fetch / HTTP** | Ler documentação oficial de gateway, canal e Prefeitura | Reduz API alucinada |
**Servidor de documentação de bibliotecas** | Assinaturas atualizadas de Next.js, Drizzle, MapLibre, ECharts | O maior ganho real: evita o agente inventar API |
**Docker** | Subir e inspecionar a stack local | — |
**Redis** | Inspecionar filas BullMQ | — |
**Cloudflare** | Cache, regras, DNS, túnel | Escopo mínimo |
**Stripe** | Sandbox, objetos de teste | Chave de **teste** apenas |
**Figma** | Se você produzir telas antes de codificar | — |

### 5.10.2 O MCP próprio — `apps/mcp` (`titan-mcp`)

Já especificado em 9.12.4. Duas instâncias distintas, nunca a mesma:

- **`titan-mcp-dev`** → aponta para banco local com dados sintéticos. É o que o agente de código usa.
- **`titan-mcp-prod`** → catálogo restrito de 9.12.4, service principals escopados, tetos no servidor, auditoria. É o que o **Hermes** consome no plano operador. **Nunca é usado por agente de desenvolvimento.**

### 5.10.3 Configuração do repositório para agente de código

- **`CLAUDE.md` na raiz:** invariantes I1–I10, anti-padrões da seção 11, e as convenções duras — dinheiro em centavos inteiros; datas de estadia como datas civis; toda Server Action valida **e** autoriza; `SET LOCAL` sempre dentro de transação; migration aplicada nunca se altera; alíquota, retenção e prazo de canal nunca em código; nenhuma rota de exclusão de evidência.
- **`CLAUDE.md` por pacote:** contrato local. Em `packages/fiscal`, o aviso de que layout e alíquota não vão para código. Em `packages/evidence`, que nada é apagável.
- **Subagentes** (`.claude/agents/`): `domain-modeler` · `migration-writer` (obrigado a gerar SQL **e** teste de constraint) · `adapter-builder` (obrigado a gerar contract test Pact) · `fiscal-specialist` · `pricing-scientist` · `security-reviewer` (roda a matriz de autorização da seção 7.3) · `a11y-reviewer` (roda axe e verifica contraste) · `frontend-designer` (aplica os tokens de 5.9 e rejeita as três coisas de 5.9.3).
- **Slash commands** (`.claude/commands/`): `/novo-adapter-canal` · `/nova-conta-contabil` · `/novo-checklist` · `/novo-agente` · `/revisar-autorizacao` · `/fechar-fase`.
- **Skills** para entregáveis de escritório: **xlsx** (modelo de repasse, apuração de ISS, projeção de fluxo de caixa, contagem de estoque) e **docx/pdf** (contrato de administração, contrato de locação por temporada, extrato do proprietário, laudo de dedetização).
- **Hooks:** `pre-commit` com typecheck, lint, `gitleaks`, teste de autorização e teste de payload de escopo; `post-edit` rodando Vitest do pacote alterado.
...
1. `/docs/adr/` — ADRs numerados. Mínimo: **0001** estilo arquitetural · **0002** topologia VPS única, disponibilidade e DR · **0003** stack e versões travadas · **0004** estratégia de canais · **0005** orquestração de pagamentos · **0006** trilha fiscal · **0007** multi-tenancy e RLS sob pooling · **0008** autenticação e autorização · **0009** hardening das instâncias de agente · **0010** por que Concierge/Vendas/Risco não rodam em Hermes/OpenClaw · **0011** integração de ponto e folha (não construir) · **0012** estratégia mobile em três níveis · **0013** captura em navegador e limitações por plataforma · **0014** fonte de dados de pricing · **0015** região Contabo, latência ao Brasil, transferência internacional de dados e gatilho do plano B no Brasil · **0016** direção visual e as três rejeições de 5.9.3 · **0017** MCPs instalados, escopos e separação dev/produção · **0018** tape chart — construir em canvas ou licenciar.
2. `/docs/domain/` — modelo de domínio, glossário pt-BR↔inglês, diagramas de contexto e máquinas de estado (Mermaid).
3. `/docs/openapi.yaml` — contrato versionado.
4. `/db/migrations/` — SQL versionado + seed com 20 unidades, 400 reservas, 12 meses de histórico, 3 proprietários, 5 prestadores, 60 SKUs e evidência de exemplo.
5. `/infra/` — docker-compose, Caddy/Traefik, PgBouncer, pgBackRest, SOPS, script de deploy sem downtime, script de restauração.
6. Código dos módulos na ordem do roadmap, com testes.
7. `/docs/integrations/` — por integração: pré-requisitos, credenciais, sandbox, mapeamento de campos, fixtures, runbook de falha.
8. `/docs/runbook.md` e `/docs/runbook-pagamentos.md` — overbooking detectado, nota rejeitada, gateway fora, canal dessincronizado, chargeback, congelamento de repasse, restauração de banco, VPS perdida.
9. `/docs/roadmap.md` — fases, estimativas, dependências externas (certificações) com prazo de risco explícito.
...
|---|---|---|
**F0** Fundação (2 sem) | Monorepo, Compose na VPS, CI, RLS multi-tenant, identity com MFA, PgBouncer, pgBackRest, observabilidade, ADRs 1–14 | Deploy sem downtime; **teste de isolamento de tenant sob pooling passa**; restauração de backup cronometrada |
...
Antes de qualquer código, entregue:
...
3. **ADR-0001 a ADR-0014** preenchidos com recomendação e justificativa.
4. **Roadmap ajustado** com dependências externas e riscos datados.
5. **Matriz de riscos** (probabilidade × impacto) com mitigação — destacando acesso a API de canal, risco fiscal, perda de VPS/backup, e dados de pricing.

Aguarde meu "ok" antes de iniciar a Fase 0. Depois, uma fase por vez, com o portão de saída verificado antes de avançar.
